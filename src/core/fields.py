...
class ReferrerField(models.URLField):
    ...
    """
    default_validators = [
        URLValidator(schemes=['http', 'https', 'android-app', ])
    ]


class ProductSlugField(models.CharField):
    description = "Extends SlugField to enforce lower-alpha-numeric slugs with no punctuation."
...
class NormalizedEmailField(models.EmailField):
    description = "Extends EmailField to enforce all-lowercase and whitespace stripping."
