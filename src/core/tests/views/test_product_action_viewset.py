...
class ProductActionViewsetTests(rest_test.APITestCase):
    ...
    def test_product_action_bad_product_slug(self):
        ...
        r = self.client.post(
            "/api/audience-users/{}/product-actions".format(au.pk), payload, format='json'
        )
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_product_action_bad_type(self):
        ...
        product_topic = mommy.make('core.ProductTopic', name="topic1")
        product = mommy.make(
            'core.Product', slug='product1', name='Product 1', brand="Govexec", type="event",
            subtypes=[product_subtype], topics=[product_topic]
        )
        product.validate_and_save()
        au = mommy.make('core.AudienceUser', email="a@a.com")
    ...
    def test_create_product_action_happy_path(self):
        ...
        self.assertEqual(
            ...
            sorted(['details', 'created', 'id', 'timestamp', 'product', 'modified', 'type'])
        )
    ...
    def test_create_product_action_happy_path_with_multiple_details(self):
        ...
        self.assertEqual([x['description'] for x in response['details']], ['details2', 'details1'])
        self.assertEqual(response['type'], 'registered')
    ...
    def test_create_and_update_product_action(self):
        ...
        r = self.client.post(
            "/api/audience-users/{}/product-actions".format(au.pk), update_payload, format='json'
        )
    ...
    def test_create_product_action_get(self):
        ...
        r = self.client.get(
            "/api/audience-users/{}/product-actions/{}"
            .format(au.pk, r.json()['id'])
        )
        self.assertEqual(r.status_code, status.HTTP_200_OK)
    ...
    def test_product_action_delete(self):
        ...
        r = self.client.delete(
            "/api/audience-users/{}/product-actions/{}"
            .format(au.pk, r.json()['id']).format(r.json().get('id'))
        )
        self.assertEqual(r.status_code, status.HTTP_501_NOT_IMPLEMENTED)
    ...
    def test_product_action_list_with_results(self):
        ...
        r = self.client.get("/api/audience-users/{}/product-actions/{}".format(
            au.pk, r.json()['id'])
        )
