...
class UserViewsetTests(rest_test.APITestCase):
    ...
    def test_create_user_with_email(self):
        email = "a@a.com"
        response = self.client.post("/api/audience-users", {"email": email}, format='json')
    ...
    def test_user_contains_expected_keys(self):
        ...
        user = r.json()
        expected_keys = [
            'id', 'email', 'created', 'modified', 'email_hash', 'omeda_id', 'sailthru_id',
            'vars', 'vars_history', 'source_signups', 'subscriptions', 'subscription_log',
            'product_actions', 'sailthru_optout'
        ]
    ...
    def test_user_malformed_source_signups_as_string(self):
        ...
        self.assertEqual(
            r.json(),
            {'source_signups': {'non_field_errors': ['Expected a list of items but got type "str".']}}
        )
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_user_source_signup_without_name(self):
        ...
        r = self.client.post("/api/audience-users", payload, format='json')
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(r.json(), {'source_signups': [{'name': ['This field is required.']}]})
    ...
    def test_user_source_signup_without_timestamp(self):
        ...
        self.assertCountEqual(user['source_signups'][0].keys(), ['timestamp', 'name',])
        self.assertIsInstance(
            isodate.parse_datetime(user['source_signups'][0]['timestamp']),
            datetime
        )
    ...
    def test_user_source_signup_with_bad_timestamp(self):
        ...
        self.assertEqual(
            r.json(),
            {'source_signups': [
                {'timestamp': ['Datetime has wrong format. Use one of these formats instead: '
    ...
    def test_user_source_signup_multiple_additions(self):
        ...
        payload = {
            "email": "a@a.com",
            "source_signups": [{"name": "foo_source"}]
        }
        r = self.client.post("/api/audience-users", payload, format='json')
        self.assertEqual(r.status_code, status.HTTP_201_CREATED)
        user = r.json()
        ...
        put_payload = {
            "source_signups": [{"name": "bar_source"}]
        }
        r = self.client.put(
        ...
        self.assertEqual(
            ...
            ['bar_source', 'foo_source']
        )

        put_payload = {
            "source_signups": [{"name": "baz_source"}]
        }
        r = self.client.put(
            "/api/audience-users/{}".format(user['id']), put_payload, format='json'
        )
    ...
    def test_user_source_signup_multiple_additions_with_timestamps(self):
        payload = {
            "email": "a@a.com",
            "source_signups": [{"name": "foo_source", "timestamp": "2016-03-29T21:15:30.249694Z"}]
        }
        r = self.client.post("/api/audience-users", payload, format='json')
        self.assertEqual(r.status_code, status.HTTP_201_CREATED)
        user = r.json()
        ...
        put_payload = {
            "source_signups": [{"name": "bar_source", "timestamp": "2015-01-28T20:10:13.249694Z"}]
        }
        r = self.client.put(
            "/api/audience-users/{}".format(user['id']), put_payload, format='json'
        )
