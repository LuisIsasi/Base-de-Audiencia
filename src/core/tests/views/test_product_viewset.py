...
class ProductViewsetTests(rest_test.APITestCase):
    ...
    def test_create_product_missing_payload(self):
        ...
        self.assertEqual(
            ...
            {
                "subtypes": [
                    "Must provide one or most product subtypes."
                ],
                "brand": [
                ...
                "topics": [
                    "Must provide one or most product topics."
                ]
            }
        )
    ...
    def test_product_list_with_results(self):
        mommy.make('core.Product', slug='foo', name='foo', type='event', brand='Govexec')
        mommy.make('core.Product', slug='bar', name='bar', type='event', brand='Govexec')
    ...
    def test_product_update(self):
        ...
        r = self.client.post("/api/products", payload, format='json')
        self.assertEqual(r.status_code, status.HTTP_201_CREATED)
        r = self.client.put("/api/products/{}".format(r.json().get('id')), {}, format='json')
        self.assertEqual(r.status_code, status.HTTP_501_NOT_IMPLEMENTED)
