...
class ListViewsetTests(rest_test.APITestCase):
    ...
    def test_subtrig_create_missing_payload(self):
        ...
        r = self.client.post("/api/lists/{}/subscription-triggers".format(list_.pk), format='json')
    ...
    def test_subtrig_create_with_missing_override_field(self):
        ...
        r = self.client.post(
            "/api/lists/{}/subscription-triggers".format(list_1.pk), payload, format='json'
        )
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(r.json(), {'override_previous_unsubscribes': 'This field is required.'})
    ...
    def test_subtrig_create_with_bad_override_field(self):
        ...
        self.assertEqual(
            r.json(),
            {"override_previous_unsubscribes": ["'foo' value must be either True or False."]}
        )
    ...
    def test_subtrig_create_with_bad_related_list_slug(self):
        ...
        payload = {'related_list_slug': 'foo2', 'override_previous_unsubscribes': 'foo'}
        r = self.client.post(
            "/api/lists/{}/subscription-triggers".format(list_.pk), payload, format='json'
        )
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(r.json(), {'related_list_slug': 'Related list does not exist.'})
    ...
    def test_subtrig_create_happy_path(self):
        ...
        r = self.client.post(
            "/api/lists/{}/subscription-triggers".format(list_foo.pk), payload, format='json'
        )
        r_json = r.json()
        self.assertEqual(r.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            sorted(r_json.keys()),
            sorted(['id', 'override_previous_unsubscribes', 'primary_list', 'related_list'])
        )
    ...
    def test_subtrig_get_view(self):
        ...
        r = self.client.get("/api/lists/{}/subscription-triggers/{}".format(list_foo.pk, st_id))
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        r_json = r.json()
    ...
    def test_subtrig_update(self):
        ...
        r = self.client.put("/api/lists/{}/subscription-triggers/{}".format(list_foo.pk, st_id))
