...
class SubscriptionTriggerSerializerTests(TestCase):
    ...
    def test_missing_primary_list_slug(self):
        ...
        self.assertEqual(cm.exception.detail, {'primary_list_slug': 'This field is required.'})

    def test_unknown_primary_list_slug(self):
        mommy.make('core.List', name="foo", slug="foo", type="newsletter")
        s = SubscriptionTriggerSerializer(data={
        ...
        with self.assertRaises(ValidationError) as cm:
            s.is_valid(raise_exception=True)
        self.assertEqual(cm.exception.detail, {'primary_list_slug': 'Primary list does not exist.'})
    ...
    def test_happy_path(self):
        ...
        self.assertEqual(
            ...
            {'related_list': list_foo, 'primary_list': list_bar, 'override_previous_unsubscribes': True}
        )
