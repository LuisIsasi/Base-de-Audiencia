...
class UserTestCase(TestCase):
    ...
    def test_vars_wrong_type_list(self):
        with self.assertRaises(ValidationError) as cm:
            AudienceUser.objects.validate_and_create(email='a@a.com', vars=['this is a list'])
    ...
    def test_vars_wrong_type_str(self):
        with self.assertRaises(ValidationError) as cm:
            AudienceUser.objects.validate_and_create(email='a@a.com', vars='this is string')
        messages = cm.exception.message_dict
        self.assertIn('vars', messages)
        self.assertIn('This field must be a dictionary.', messages['vars'])
    ...
    def test_list_subscribe_happy_path(self):
        ...
        self.assertIsInstance(au.subscriptions.get(list__slug='foo', active=True), Subscription)
    ...
    def test_list_unsubscribe_happy_path(self):
        ...
        self.assertEqual(au.subscriptions.all().count(), 1)
        self.assertIsInstance(au.subscriptions.get(list__slug='foo', active=False), Subscription)
    ...
    def test_record_product_action_bogus_action_type(self):
        ...
        self.assertEqual(
            ...
            ["Value 'bar' is not a valid choice."]
        )
    ...
    def test_record_product_action_bogus_product_slug(self):
        ...
        self.assertRaises(
            ...
            au.record_product_action, 'foo', 'consumed', timezone.now())
    ...
    def test_record_product_action_with_details_as_str(self):
        ...
        self.assertRaises(TypeError,
            au.record_product_action, 'foo', 'consumed', timezone.now(), 'test details'
        )
