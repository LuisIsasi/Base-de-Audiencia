...
class ListTestCase(TestCase):
    ...
    def test_slug_empty_str(self):
        list_instance = mommy.make('core.List', name='newsletter name', type='newsletter', slug='')
        with self.assertRaises(ValidationError) as cm:
            list_instance.full_clean()
        self.assertIn('slug', cm.exception.message_dict.keys())
    ...
    def test_slug_missing(self):
        try:
            mommy.make('core.List', name='newsletter name', type='newsletter', slug=None)
    ...
    def test_name_missing(self):
        try:
            mommy.make('core.List', slug='newsletter_name', type='newsletter', name=None)
        except Exception as e:
            # not catching the exception explicitly to avoid coupling to the db backend,
            # and instead doing this weird class name test
            self.assertEqual(e.__class__.__name__, 'IntegrityError')
    ...
    def test_type_missing(self):
        try:
            mommy.make('core.List', slug='newsletter-name', name='newsletter name', type=None)
        except Exception as e:
            # not catching the exception explicitly to avoid coupling to the db backend,
            # and instead doing this weird class name test
            self.assertEqual(e.__class__.__name__, 'IntegrityError')
    ...
    def test_type_invalid(self):
        list_instance = mommy.make('core.List', name='newsletter name', type='foo', slug='fake_slug')
        with self.assertRaises(ValidationError) as cm:
            list_instance.full_clean()
        self.assertIn('type', cm.exception.message_dict.keys())
        self.assertIn("Value 'foo' is not a valid choice.", cm.exception.message_dict['type'])
    ...
    def test_clean(self):
        list_foo = mommy.make(
            'core.List', slug='noodles', name='a', type='list'
        )
        invalids = ['newsletter_noodles', 'custom_newsletter_noodles' 'archive_newsletter_noodles']
