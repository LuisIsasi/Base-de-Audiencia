...
class ProductTestCase(TestCase):
    ...
    def test_slug_is_none(self):
        ...
        self.assertEqual(list(cm.exception.message_dict.keys()), ["slug"])
        self.assertEqual(
            cm.exception.message_dict["slug"], ["This field cannot be null."]
        )
    ...
    def test_name_is_empty_string(self):
        product = mommy.make(
            "core.Product", slug="foo", brand="Govexec", type="event", name=""
        )
        with self.assertRaises(ValidationError) as cm:
            product.full_clean()
        self.assertEqual(list(cm.exception.message_dict.keys()), ["name"])
    ...
    def test_brand_is_none(self):
        product = mommy.make(
            "core.Product", name="foo", slug="foo", brand="Govexec", type="event"
        )
        product.brand = None
        with self.assertRaises(ValidationError) as cm:
            product.full_clean()
        ...
        self.assertEqual(
            cm.exception.message_dict["brand"], ["This field cannot be null."]
        )

    def test_brand_is_empty_string(self):
        product = mommy.make(
            "core.Product", name="foo", slug="foo", brand="", type="event"
        )
        with self.assertRaises(ValidationError) as cm:
            product.full_clean()
        self.assertEqual(list(cm.exception.message_dict.keys()), ["brand"])
        self.assertEqual(
            cm.exception.message_dict["brand"], ["This field cannot be blank."]
        )

    def test_brand_unexpected_value(self):
        product = mommy.make(
            "core.Product", name="foo", slug="foo", brand="Foo", type="event"
        )
        with self.assertRaises(ValidationError) as cm:
            product.full_clean()
        self.assertEqual(list(cm.exception.message_dict.keys()), ["brand"])
        self.assertEqual(
            cm.exception.message_dict["brand"], ["Value 'Foo' is not a valid choice."]
        )
    ...
    def test_type_is_none(self):
        ...
        self.assertEqual(
            cm.exception.message_dict["type"], ["This field cannot be null."]
        )
    ...
    def test_type_is_empty_string(self):
        ...
        self.assertEqual(list(cm.exception.message_dict.keys()), ["type"])
        self.assertEqual(
            cm.exception.message_dict["type"], ["This field cannot be blank."]
        )

    def test_type_unexpected_value(self):
        product = mommy.make(
            "core.Product", name="foo", slug="foo", brand="Govexec", type="foo"
        )
        with self.assertRaises(ValidationError) as cm:
            product.full_clean()
        self.assertEqual(list(cm.exception.message_dict.keys()), ["type"])
        self.assertEqual(
            cm.exception.message_dict["type"], ["Value 'foo' is not a valid choice."]
        )
