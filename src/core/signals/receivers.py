...
@receiver(
    ...
    dispatch_uid="core::signals::record_user_vars_history",
)
def record_user_vars_history(sender, **kwargs):
    user = kwargs.get("instance")
    if user:
        history = core_models.UserVarsHistory.objects.filter(
            audience_user=user
        ).order_by("-timestamp")
        if not history:
...
@receiver(
    post_save,
    sender=core_models.AudienceUser,
    dispatch_uid="core::signals::update_var_keys",
)
...
@receiver(
    post_save,
    sender=core_models.SubscriptionLog,
    dispatch_uid="core::signals::triggered_subscribes",
)
def triggered_subscribes(sender, **kwargs):
    ...
    for trigger in list_.subscription_triggers.all():
        ...
        ok_to_subscribe = (
            True
            if trigger.override_previous_unsubscribes
            else (related_list not in currently_unsubscribed_lists)
        )
