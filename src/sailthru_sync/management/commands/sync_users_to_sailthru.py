...
class Command(BaseCommand):
    ...
    def _validate_options(self, options):
        ...
        if options["user"] and options["range"]:
            raise CommandError(
                "Supply only one of the following arguments: '--user', '--range'."
            )
        ...
        if options["range"] and options["range"][0] != "all":
            ...
            if invalid_range:
                raise CommandError(
                    '--range must specify either "all" or a slice like "0:500".'
                )
    ...
    def handle(self, *args, **options):
        ...
        if options["user"]:
            ...
            if not user.email:
                raise CommandError(
                    "Cannot sync users that do not have email addresses."
                )
