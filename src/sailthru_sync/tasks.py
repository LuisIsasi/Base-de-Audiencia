...
@throttle(interval_seconds=settings.SAILTHRU_TASK_THROTTLE_INTERVAL, logger_name=__name__)
def sync_user_basic(self, user_pk):
    ...
    try:
        ...
        if lock.age > max_age:
            ...
            with sentry_sdk.push_scope() as scope:
                ...
                scope.set_extra("lock", {
                    "pk": lock.pk,
                    "Age (minutes)": lock.age.total_seconds() / 60,
                    "Max age (minutes)": max_age.total_seconds() / 60,
                })
                sentry_sdk.capture_message(msg)
            logger.warn("Sailthru sync basic: Sync lock %s for user %s is old.", str(lock.pk), str(user_pk))
        else:
            logger.warn("Sailthru sync basic: Sync lock exists for user %s.", str(user_pk))
            self.retry(countdown=settings.SAILTHRU_TASK_THROTTLE_INTERVAL + 1)  # This should hopefully ensure we run a sync event without hitting a stale update lock
    ...
    try:
        ...
        sentry_sdk.capture_exception(e)
        logger.error("Sailthru sync basic: Problem occured during request to Sailthru: %s", str(e))
    ...
    if not response.is_ok():
        ...
        logger.error("Sailthru sync basic: Sailthru rejected request to sync for user %s (%s)", str(aud_user.pk), aud_user.email)
        return
    ...
    try:
        ...
        logger.error("Sailthru sync basic: Sailthru response missing expected values on user %s (%s).", str(aud_user.pk), aud_user.email)
    else:
        changed_sid = False
        ...
        if changed_sid:
            ...
            logger.error(msg)
            return
    logger.info("Finished sailthru sync for user %s (%s).", str(aud_user.pk), aud_user.email)


@celery_app.task(bind=True)
@log_on_error("Send sync failure notifications: unhandled exception.")
def send_sync_failure_notifications(self):
    logger.info("Send sync failure notifications: Checking if there are any groups to notify...")
    groups = (
        m.SyncFailureNotificationGroup.objects
        .prefetch_related("interested_group", "interested_group__user_set")
        .all()
    )

    if not groups:
        logger.info("Send sync failure notifications: There are no groups interested in notifications.")
        logger.info("Send sync failure notifications: Complete.")
        return

    logger.info("Send sync failure notifications: Checking if there are any failures of interest...")
    interested_errors = {
        error
        for group in groups
    ...
    failures = m.SyncFailure.objects.filter(
        acknowledged=False,
        resolved=False,
        sailthru_error_code__in=interested_errors
    )

    if not failures:
        logger.info("Send sync failure notifications: There are no failures of interest.")
        logger.info("Send sync failure notifications: Complete.")
        return
    ...
    for group in groups:
        ...
        if not interested_failures:
            continue
        subject = '[Audb][Sync failures][{}] Notification'.format(group.interested_group.name)
        message = construct_message_from_failures(interested_failures)
        emails = set()
        for user in group.interested_group.user_set.all():
            if user.email:
                emails.add(user.email)
        logger.info("Send sync failure notifications: Sending email to group '{}'...".format(group.interested_group.name))
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, list(emails), fail_silently=False)
...
def construct_message_from_failures(failures):
    ...
    sorted_failures = sorted([
        (hr_errors[failure.sailthru_error_code], failure)
        for failure in failures
    ], key=lambda x: x[0])
